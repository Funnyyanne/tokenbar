...
class ClaudeAdapter(Adapter):
    ...
    def __init__(self, roots: list[Path] | None = None, max_bytes: int = 4 * 1024 * 1024) -> None:
        self.roots = roots or [Path(os.environ.get("CLAUDE_CONFIG_DIR", Path.home() / ".claude")) / "projects"]
        self.max_bytes = max_bytes

    def snapshot(self) -> Snapshot:
        files = newest_files(self.roots)
        if not files:
            return Snapshot.unavailable(self.provider, self.label, "未找到 Claude 会话日志", str(self.roots[0]))
        path = files[0]
